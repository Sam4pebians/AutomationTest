...
export class CommonPage extends BasePage {
    ...
    static getAllTableColumnHeaders(tableNameClassOrId: string) {
        const columnSelector = By.css(`.${tableNameClassOrId}:last-of-type th`);
        return element.all(columnSelector);
    }

    static getColumnBasedOnAnotherColumn(tableNameClassOrId: string, referenceColumnValue: string, columnIndex: number) {
        const columnSelector = By.xpath(`
        //table[@id="${tableNameClassOrId}"]//td[normalize-space(.)="${referenceColumnValue}"]//ancestor::tr//td[${columnIndex}]`);
    ...
    static getAllAnchorsByClass (classAttributeValue: string, classIsContains = false, insidePopup = false) {
        return element.all (By.xpath (AnchorComponentSelectors.getAnchorByClassXpath (classAttributeValue, classIsContains, insidePopup)));
    }
    ...
    static getAllLabelByText (text: string, textIsContains = false, insidePopup = false) {
        return element.all (By.xpath (TextComponentSelectors.getLabelByTextXpath (text, textIsContains, insidePopup)));
    }

    static getLabelByText (text: string, textIsContains = false, insidePopup = false) {
        return this.getAllLabelByText(text, textIsContains, insidePopup).first();
    }

    static getLabelContainsText (text: string) {
        return this.getLabelByText(text, true);
    }

    static getAllTdByText (text: string, textIsContains = false, insidePopup = false) {
        return element.all (By.xpath (TextComponentSelectors.getTdByTextXpath (text, textIsContains, insidePopup)));
    }

    static getTdByText (text: string, textIsContains = false, insidePopup = false) {
        return this.getAllTdByText(text, textIsContains, insidePopup).first();
    }

    static getTdContainsText (text: string) {
        return this.getTdByText(text, true);
    }
    ...
    static getAllHeaderOneByText(text: string, textIsContains = false, insidePopup = false) {
        return element.all(By.xpath(TextComponentSelectors.getHeaderOneByTextXpath(text, textIsContains, insidePopup)));
    }

    static getHeaderOneByText(text: string, textisContains = false, insidePopup = false) {
        return this.getAllHeaderOneByText(text, textisContains, insidePopup).first();
    }

    static getHeaderOneContainingText(text: string) {
        return this.getHeaderOneByText(text, true);
    }

    static getAllHeaderThreeByText(text: string, textIsContains = false, insidePopup = false) {
        return element.all(By.xpath(TextComponentSelectors.getHeaderThreeByTextXpath(text, textIsContains, insidePopup)));
    }

    static getHeaderThreeByText(text: string, textisContains = false, insidePopup = false) {
        return this.getAllHeaderThreeByText(text, textisContains, insidePopup).first();
    }

    static getSecondHeaderThreeByText(text: string, textisContains = false, insidePopup = false) {
        return this.getAllHeaderThreeByText(text, textisContains, insidePopup).get(1);
    }

    static getHeaderThreeContainsText(text: string) {
        return this.getHeaderThreeByText (text, true);
    }
    ...
    static getAllButtonByFormIdAndInputNameXpath(
        idAttributeValue: string, nameAttributeValue: string, idContains = false, nameContains = false, insidePopup = false) {
        return element.all(By.xpath(ButtonComponentSelectors.getButtonByFormIdAndInputNameXpath(
            idAttributeValue, nameAttributeValue, idContains, nameContains , insidePopup)));
    }

    static getButtonByFormIdAndInputNameXpath (
        idAttributeValue: string, nameAttributeValue: string, idContains = false, nameContains = false, insidePopup = false) {
        return this.getAllButtonByFormIdAndInputNameXpath(
            idAttributeValue, nameAttributeValue, idContains, nameContains, insidePopup ).first();
    }

    static getButtonContainsFormIdAndInputNameXpath (idAttributeValue: string, nameAttributeValue: string) {
        return this.getButtonByFormIdAndInputNameXpath(idAttributeValue, nameAttributeValue, true, true);
    }

    static getAllSpanTextBytDivId(parentDivId: string,
                                  isContainsId = false,
                                  spanText: string,
                                  isContainsText = false,
                                  insidePopup = false) {
        return element.all(By.xpath(TextComponentSelectors.getSpanTextBytDivIdXpath(parentDivId,
            isContainsId, spanText, isContainsText, insidePopup)));
    }

    static getSpanTextBytDivId (parentDivId: string,
                                isContainsId = false,
                                spanText: string,
                                isContainsText = false,
                                insidePopup = false) {
        return this.getAllSpanTextBytDivId(parentDivId,
            isContainsId, spanText, isContainsText, insidePopup).first();
    }

    static getSpanContainingTextBytDivIdXpath(parentDivId: string,
                                              isContainsId = false,
                                              spanText: string) {
        return this.getSpanTextBytDivId(parentDivId,
            isContainsId, spanText, true);
    }

    static getAllHeaderTwoByText(text: string, textIsContains = false, insidePopup = false) {
        return element.all(By.xpath(TextComponentSelectors.getHeaderTwoByTextXpath(text, textIsContains, insidePopup)));
    }

    static getHeaderTwoByText(text: string, textIsContains = false, insidePopup = false) {
        return this.getAllHeaderTwoByText(text, textIsContains, insidePopup).first();
    }

    static getHeaderTwoContainingText(text: string) {
        return this.getHeaderTwoByText(text, true);
    }

    static getAllHeaderFourByText(text: string, textIsContains = false, insidePopup = false) {
        return element.all(By.xpath(TextComponentSelectors.getHeaderFourByTextXpath(text, textIsContains, insidePopup)));
    }

    static getHeaderFourByText(text: string, textIsContains = false, insidePopup = false) {
        return this.getAllHeaderFourByText(text, textIsContains, insidePopup).first();
    }

    static getHeaderFourContainingText(text: string) {
        return this.getHeaderFourByText(text, true);
    }

    static getAllButtonInsideTableUsingText(tableElementText: string,
                                            isTableElementTextContains = false,
                                            buttonName: string,
                                            isButtonNameContains = false,
                                            insidePopup = false) {
        return element.all(By.xpath(ButtonComponentSelectors.getButtonInsideTableUsingTextXpath(
            tableElementText, isTableElementTextContains, buttonName, isButtonNameContains, insidePopup)));
    }

    static getButtonInsideTableUsingText(tableElementText: string,
                                         isTableElementTextContains = false,
                                         buttonName: string,
                                         isButtonNameContains = false,
                                         insidePopup = false) {
        return this.getAllButtonInsideTableUsingText(
            tableElementText, isTableElementTextContains, buttonName, isButtonNameContains, insidePopup).first();
    }

    static getButtonInsideTableContainingText(tableElementText: string,
                                              isTableElementTextContains = false,
                                              buttonName: string) {
        return this.getButtonInsideTableUsingText(
            tableElementText, isTableElementTextContains, buttonName, true);
    }

    static getAllAnchorInsideTableUsingText(tableElementText: string,
                                            isTableElementTextContains = false,
                                            anchorText: string,
                                            isAnchorTextContains = false,
                                            insidePopup = false) {
        return element.all(By.xpath(AnchorComponentSelectors.getAnchorInsideTableUsingTextXpath(
            tableElementText, isTableElementTextContains, anchorText, isAnchorTextContains, insidePopup)));
    }

    static getAnchorInsideTableUsingText(tableElementText: string,
                                         isTableElementTextContains = false,
                                         anchorText: string,
                                         isAnchorTextContains = false,
                                         insidePopup = false) {
        return this.getAllAnchorInsideTableUsingText(
            tableElementText, isTableElementTextContains, anchorText, isAnchorTextContains, insidePopup).first();
    }

    static getAnchorInsideTableContainingText(tableElementText: string,
                                              isTableElementTextContains = false,
                                              anchorText: string) {
        return this.getAnchorInsideTableUsingText(
            tableElementText, isTableElementTextContains, anchorText, true);
    }

    static getAllSpanInsideTableUsingText(tableElementText: string,
                                          isTableElementTextContains = false,
                                          spanText: string,
                                          isSpanTextContains = false,
                                          insidePopup = false) {
        return element.all(By.xpath(TextComponentSelectors.getSpanInsideTableUsingTextXpath(
            tableElementText, isTableElementTextContains, spanText, isSpanTextContains, insidePopup)));
    }

    static getSpanInsideTableUsingText(tableElementText: string,
                                       isTableElementTextContains = false,
                                       spanText: string,
                                       isSpanTextContains = false,
                                       insidePopup = false) {
        return this.getAllSpanInsideTableUsingText(
            tableElementText, isTableElementTextContains, spanText, isSpanTextContains, insidePopup).first();
    }

    static getSpanInsideTableContainingText(tableElementText: string,
                                            isTableElementTextContains = false,
                                            spanText: string) {
        return this.getSpanInsideTableUsingText(
            tableElementText, isTableElementTextContains, spanText, true);
    }

    static getAllFollowingSiblingOfTableElementByText(tableElementText: string,
                                                      isTableElementTextContains = false,
                                                      siblingText: string,
                                                      isSiblingTextContains = false,
                                                      insidePopup = false) {
        return element.all(By.xpath(GridComponentSelectors.getFollowingSiblingOfTableElementByTextXpath(
            tableElementText, isTableElementTextContains, siblingText, isSiblingTextContains, insidePopup)));
    }

    static getFollowingSiblingOfTableElementByText(tableElementText: string,
                                                   isTableElementTextContains = false,
                                                   siblingText: string,
                                                   isSiblingTextContains = false,
                                                   insidePopup = false) {
        return this.getAllFollowingSiblingOfTableElementByText(
            tableElementText, isTableElementTextContains, siblingText, isSiblingTextContains, insidePopup).first();
    }

    static getFollowingSiblingOfTableElementContainingText(tableElementText: string,
                                                           isTableElementTextContains = false,
                                                           siblingText: string) {
        return this.getFollowingSiblingOfTableElementByText(
            tableElementText, isTableElementTextContains, siblingText, true, false);
    }

    static getAllFollowingSiblingOfTableElementByClassName(tableElementText: string,
                                                           isTableElementTextContains = false,
                                                           className: string,
                                                           isContainsClassName = false,
                                                           insidePopup = false) {
        return element.all(By.xpath(GridComponentSelectors.getFollowingSiblingOfTableElementByClassXpath(
            tableElementText, isTableElementTextContains, className, isContainsClassName, insidePopup)));
    }

    static getFollowingSiblingOfTableElementByClassName(tableElementText: string,
                                                        isTableElementTextContains = false,
                                                        className: string,
                                                        isContainsClassName = false,
                                                        insidePopup = false) {
        return this.getAllFollowingSiblingOfTableElementByClassName(
            tableElementText, isTableElementTextContains, className, isContainsClassName, insidePopup).first();
    }

    static getFollowingSiblingOfTableElementContainingClassName(tableElementText: string,
                                                                isTableElementTextContains = false,
                                                                className: string) {
        return this.getFollowingSiblingOfTableElementByClassName(
            tableElementText, isTableElementTextContains, className, true, false);
    }

    static getNameTextBox() {
        return element(By.id(CommonPageConstants.labelIds.nameId));
    }
}
